...
# Start building the model ______________________________________________________________________________

runs = 4 # Total number of times that a specific model is solved
max_time = 35 # Max time the solver has to solve the problem (in seconds)

rho_vector = np.array([[0.3, 0.4, 0.5]]) # Different values of correlations
epsilon_vector = np.array([0.1]) # Value of reliability level (in epsilon)

max_scenarios = 20000 # Maximum number of scenarios the probem will be solved with
number_of_points = 25 # Number of scenarios values the problem will be solved
...
for k in range(number_of_points):
    ...
    for r in range(len(rho_vector)):
        ...
        for l in range(len(epsilon_vector)):
            ...
            for i in range(N):
                    for t in range(T):
                            cqm.add_constraint(p[0,t] + p[1,t] + p[2,t] - Z_transformed[i,t]*y[i] >= 0)
            ...
            for i in range(G):
                for t in range(T):
                    ...
                    if t != 0:
                        ...
                        cqm.add_constraint(u[i,t] - z_on[i,t] + z_off[i,t] == u_initial[i])                    
                        cqm.add_constraint(p[i,t] - R_startup[i]*z_on[i,t] <= p_initial[i] + R_up[i]*u_initial[i])
                        cqm.add_constraint(-p[i,t] - R_down[i]*u[i,t] - R_shutdown[i]*z_off[i,t] <= -p_initial[i])

            cqm.set_objective(sum([C_shutdown[i]*z_off[i,t] + C_startup[i]*z_on[i,t] + b[i]*p[i,t] + c[i]*u[i,t] for i in range(G) for t in range(T)]))
            
            save_model_in_json(cqm, f"instances/CQM_{N}scenarios_{r}rho_{l}epsilon.json")

            sampler = LeapHybridCQMSampler(token = "introduce_your_token")
            sampler.properties["minimum_time_limit_s"] = 35

            serialized_sampleset = save_data(sampler = sampler, runs = runs, max_time = max_time, m = cqm, label = f"{N}scenarios_{k}rho_{l}epsilon")
            with open(f"results/results_cost_vs_scenarios_{N}scenarios_{k}rho_{l}epsilon.json", "w") as f:
